    ...
    <section id="promocoes" aria-labelledby="promocoes-titulo" className="py-20 bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {promocoes.map((promo) => (
            <Card key={(promo as any)._id || promo.id} className="p-6 relative bg-card hover:border-accent/30 transition-colors flex flex-col h-full border-border">
              {promo.destaque && (
              ...
              <div className="pt-4 border-t border-border mt-auto">
                ...
                <Link to={`/${tenant.slug}/agendar`} className="block w-full">
                  <Button className={`w-full ${promo.destaque ? 'bg-accent text-background hover:bg-accent/90' : 'border-border text-foreground hover:bg-accent hover:text-background border'}`} variant={promo.destaque ? 'default' : 'outline'}>
                    Aproveitar Oferta
                  </Button>
                </Link>
              </div>
